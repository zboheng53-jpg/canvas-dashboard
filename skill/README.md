...
课程作业等普通责任默认进入待办，即使每周/双周重复也不因此建项目。默认记录每项最近一次明确截止，周期说明保存在详情；当前待办完成后不会自动生成下次，重复日程也不能替代重复作业。只有明确要求批量录入时才展开各次。

接入后每项目默认只落实当前一步，方向和训练方法放资料，完成后不自动续写任务链。用户明确要求多步时可以扩展；服务端不设置任务数量上限。

更新后在新会话检查：询问“帮我推进英语项目”，Agent 应先读取项目与今日行动，复用已有下一步，不自动创建每日训练和复盘。MCP 用户也需更新下载包并重启接入进程；只更新服务器不会替换本机旧脚本。

也可用“整理三门课的作业进待办，其中两项每周/双周重复”验证：应在普通待办看到每项最近截止，不出现新建课业项目、整学期任务展开或虚构的学期末截止。双周基准不明时，只澄清那一项。该检查需要在示例账户进行，避免生成真实测试任务。
