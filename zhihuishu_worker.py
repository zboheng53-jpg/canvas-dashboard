...
def _run_loop(usernames: list[str], dry_run: bool = False):
    failures = {username: 0 for username in usernames}
    last_fetch = {username: 0.0 for username in usernames}
    while True:
        for username in usernames:
            try:
                ok = _run_once(username, dry_run=True) if dry_run else run_scheduled_cycle(username)
                failures[username] = 0 if ok else failures[username] + 1
                last_fetch[username] = time.time()
    ...
    try:
        with single_instance_lock():
            if args.once:
                ok = True
                for username in usernames:
                    ok = _run_once(username, dry_run=args.dry_run) and ok
                return 0 if ok else 1
            _run_loop(usernames, dry_run=args.dry_run)
